...
| Visual hierarchy | 5/5 | The page moves from role and verified facts to origin, process, production evidence, responsibility, customer fit, location, and a final two-path CTA. |
...
| Responsive behavior | 4/5 | Desktop and narrow mobile checks showed no horizontal overflow; hero actions, factory media, and closing CTAs stack correctly and remain above the sticky contact bar. A dedicated 200% zoom audit remains advisable before production. |
...
| Design-system consistency | 5/5 | The implementation uses the canonical yellow, dark proof surfaces, square editorial media, concise Thai headings, and action hierarchy without decorative gradients, badges, or nested rounded cards. |
