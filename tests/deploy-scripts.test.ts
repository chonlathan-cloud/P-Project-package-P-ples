...
const deploymentScripts = [
  ...
  ["deploy_web_prod.sh", "mode=prod service=web mutation=build-candidate-verify-promote"],
  ["deploy_api_test.sh", "mode=test service=api mutation=build-and-deploy"],
  ["deploy_api_prod.sh", "mode=prod service=api mutation=build-candidate-verify-promote"],
] as const;

describe("deployment entrypoints", () => {
  it.each(deploymentScripts)("prints a non-mutating plan for %s", (script, expected) => {
  ...
  it("keeps the former Web Test command and positional tag compatible", () => {
    const output = execFileSync("bash", ["deploy-web-test.sh", "legacy-test-tag", "--dry-run"], {
      cwd: repositoryRoot,
      encoding: "utf8",
      timeout: 3_000,
    });
